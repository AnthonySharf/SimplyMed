export default function RedFlags({ events }) {
  return (
    <div>
      <ul className="space-y-4">
        {events.map(([name, { frequency }]) => {
          const display = name.replace(/^⚠️\s*/, '')
          return (
            <li key={name} className="flex items-center gap-4">
              <span className="text-white text-sm w-52 shrink-0">{display}</span>
              <div className="flex-1 bg-[#2A2A2A] rounded-full h-1.5">
                <div className="bg-red-500 h-1.5 rounded-full" style={{ width: `${frequency}%` }} />
              </div>
